                ...
                card_wrapper([
                    html.Div(
                        style={"display": "flex", "justifyContent": "space-between", "alignItems": "flex-end",
                               "flexWrap": "wrap", "gap": "12px"},
                        children=[
                            section_header("Pipeline Flow — Sankey Diagram",
                                           "Trace how data flows from source file → sample → state → gene segment → QC status"),
                            html.Div([
                                html.Label("QC Threshold (TASS Score)", style=dropdown_label_style()),
                                dcc.Input(
                                    id="sankey-threshold",
                                    type="number", value=95, min=0, max=100, step=0.5,
                                    style={
                                        "width": "100px", "backgroundColor": COLORS["bg"],
                    ...
                    dcc.Graph(id="sankey-chart", figure=empty_fig(), config={"displayModeBar": False}),
                ]),

                # ── Row 7: Custom chart builder ──
                card_wrapper([
                    html.Div(
...
def update_sankey(store_data, threshold, samples, states, segments):
    dff = filtered_from_store(store_data, samples, states, segments)
    if dff is None or dff.empty:
        return empty_fig()

    threshold = threshold if threshold is not None else 95

    # Assign QC status per row
    dff = dff.copy()
    dff["QC Status"] = dff["TASS Score"].apply(lambda x: "PASS" if x >= threshold else "FAIL")

    # ── Build node list: Source File → Sample → State → Gene Segment → QC Status ──
    has_source = "_source_file" in dff.columns and dff["_source_file"].nunique() > 1
    nodes = []
    node_index = {}
    ...
    # Decide flow layers based on whether multi-file
    if has_source:
        # Layer 1: Source File → Sample
        grp = dff.groupby(["_source_file", "Sample"]).size().reset_index(name="count")
        for _, row in grp.iterrows():
            s = get_node(row["_source_file"], "file")
            t = get_node(row["Sample"], "sample")
            sources.append(s)
            targets.append(t)
            values.append(row["count"])
            link_colors.append("rgba(0,212,170,0.18)")

    # Layer 2: Sample → State
    grp = dff.groupby(["Sample", "state"]).size().reset_index(name="count")
    for _, row in grp.iterrows():
        s = get_node(row["Sample"], "sample")
        t = get_node(row["state"], "state")
        sources.append(s)
        targets.append(t)
        values.append(row["count"])
        link_colors.append("rgba(124,58,237,0.18)")

    # Layer 3: State → Gene Segment
    grp = dff.groupby(["state", "Gene Segment"]).size().reset_index(name="count")
    for _, row in grp.iterrows():
        s = get_node(row["state"], "state")
        t = get_node(row["Gene Segment"], "segment")
        sources.append(s)
        targets.append(t)
        values.append(row["count"])
        link_colors.append("rgba(245,158,11,0.18)")

    # Layer 4: Gene Segment → QC Status
    grp = dff.groupby(["Gene Segment", "QC Status"]).size().reset_index(name="count")
    for _, row in grp.iterrows():
        s = get_node(row["Gene Segment"], "segment")
        t = get_node(row["QC Status"], "qc")
        sources.append(s)
        targets.append(t)
        values.append(row["count"])
        if row["QC Status"] == "PASS":
            ...
            link_colors.append("rgba(239,68,68,0.25)")

    # Node colors by group
    group_colors = {
        "file": COLORS["accent"],
        "sample": COLORS["accent2"],
        "state": COLORS["accent3"],
        "segment": "#3b82f6",
        "qc": None,  # handled per-node below
    }

    node_colors = []
    for n in nodes:
        if n["group"] == "qc":
            node_colors.append(COLORS["success"] if n["label"] == "PASS" else COLORS["danger"])
        else:
            node_colors.append(group_colors.get(n["group"], "#888"))
    ...
    fig = go.Figure(go.Sankey(
        ...
        node=dict(
            ...
            color=node_colors,
            line=dict(color=COLORS["card_border"], width=0.5),
